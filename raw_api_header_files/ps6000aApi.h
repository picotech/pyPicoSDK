...
// PICO_OK
// PICO_INVALID_HANDLE (handle isn't that of an open PicoScope device.)
// PICO_SIGGEN_TRIGGERSOURCE_NOT_SUPPORTED (The trigger source is not listed in enSigGenTrigSource.)
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps6000aSetAuxIoMode)(int16_t handle, PICO_AUXIO_MODE auxIoMode);


// This function sets the holdoff counter by samples.
// This function sets the trigger holdoff given a sample count. Trigger holdoff allows you to set a period when the
// scope won't look for further trigger events after each triggered acquisition.
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps6000aSetTriggerHoldoffCounterBySamples)
(
	int16_t handle, 
	uint64_t samples
	);
...
// The introduction of API function ps6000aSetAuxIoMode has superseded the auxOutputEnable parameter and is no longer used in the driver.
